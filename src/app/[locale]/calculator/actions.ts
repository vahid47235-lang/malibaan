...
export async function submitQuoteLeadAction(
  ...
  if (name.length < 2) return { error: dict.common.forms.errorNameTooShort };
  if (!/^0?9\d{9}$/.test(phone.replace(/[\s-]/g, ""))) {
    return { error: dict.common.forms.errorPhoneInvalid };
  }
  if (selectedServiceIds.length === 0) {
    return { error: dict.calculator.selectAtLeastOne };
  }
